...
echo $bold.$red."[-] By Manuk Mimin \n\n".$cln;
    ...
    echo "===============\n\n";
    echo $bold . $lblue . "•> $fgreen [$white 1 $fgreen]$cln Auto Reff AdaOTP$cln\n";
    echo $bold . $lblue . "•> $fgreen [$white 2 $fgreen]$cln Manual reff OTP$cln\n";
...
if (strpos($getToken[1], '"code":200,')) {
    ...
    if (strpos($request_login[1], '"redirect_url":')) {
		...
		$jml = $bold.$orange."[-] Jumlah WD ".$cln."(Fee Rp 4.620)";
			...
			if ($mes_wd == 'Kode OTP Salah, Mohon cek kembali kode OTP Cok') {
                        echo $bold.$red."[-] ".$cln.$mes_wd.PHP_EOL.PHP_EOL;
			die;
			}
			if ($mes_wd == 'created') {
                        echo $bold.$fgreen."[-] Berhasil Withdraw kontol".$cln.PHP_EOL;
			die;
			} else {
